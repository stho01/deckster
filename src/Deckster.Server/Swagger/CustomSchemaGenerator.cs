...
public class DecksterSchemaGeneratorForDealingWithSwaggerImbecility : ISchemaGenerator
    {
        ...
        public DecksterSchemaGeneratorForDealingWithSwaggerImbecility(SchemaGeneratorOptions generatorOptions, ISerializerDataContractResolver serializerDataContractResolver)
            : this(generatorOptions, serializerDataContractResolver, null)
        {
        }

        public DecksterSchemaGeneratorForDealingWithSwaggerImbecility(SchemaGeneratorOptions generatorOptions, ISerializerDataContractResolver serializerDataContractResolver, IOptions<MvcOptions> mvcOptions)
        {
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private OpenApiSchema CreateObjectSchema(DataContract dataContract, SchemaRepository schemaRepository)
        {
            ...
            var applicableDataProperties = dataContract.ObjectProperties.Where(p => p.MemberInfo.DeclaringType == dataContract.UnderlyingType && !p.MemberInfo.IsOverridden());
            ...
            if (_generatorOptions.UseAllOfForInheritance || _generatorOptions.UseOneOfForPolymorphism)
            {
                ...
                if (IsBaseTypeWithKnownTypesDefined(dataContract, out var knownTypesDataContracts))
                {
                    ...
                    {
                        var prop = dataContract.UnderlyingType.GetProperty(discriminator.PropertyName, BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly |BindingFlags.IgnoreCase);
                        if (prop != null && !prop.IsOverridden())
                        {
