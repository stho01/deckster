...
public class Program
{
    ...
    private static async Task GenerateOpenApiSchemaForMessagesAsync(DirectoryInfo gitDirectory)
    {
        ...
        await openapi.WriteAsYamlAsync(gitDirectory.GetFile("generated", "deckster.opeanpi.yaml"));
    }
    ...
    private static async Task GenerateClientsAsync(DirectoryInfo gitDirectory)
    {
        ...
        var kotlinDirectory = gitDirectory.GetCleanSubDirectory("generated", "kotlin");
    ...
    private static async Task GenerateCsharpAsync(DirectoryInfo gitDirectory, Type gameType, CSharpGameMeta game)
    {
        ...
        var csharp = new CsharpClientGenerator(game, $"Deckster.Client.Games.{ns}");
        
        var clientDirectory = gitDirectory.GetCleanSubDirectory("src", "Deckster.Client", "Games", game.Name);
        await csharp.WriteToAsync(clientDirectory.GetFile($"{game.Name}Client.g.cs"));
    }
